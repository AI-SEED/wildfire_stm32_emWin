	 ...
	 static char move_flag = 0;
	  ...
	  switch (pMsg->MsgId) {
		...
		case WM_CREATE:
			for(i=0;i<110;i++)
				{
				 // BUTTON_CreateEx(PITCH+i*18,PITCH,18,20,hWordWin,WM_CF_SHOW,NULL,ID_TEXT+i);
		...
		switch(NCode){
				...
				if(Id>=ID_TEXT && Id<ID_TEXT+110)
					{
						...
						if(IdSel == 0 && (move_flag != 1)&& (strlen(t9pyfun.t9PY_addr->PY_mb)>PYSel*3))
						{								
			...
			//case WM_NOTIFICATION_MOVED_OUT:		//MOVE OUT 和release同样处理
			case WM_NOTIFICATION_RELEASED:
				...
				if(Id>=ID_TEXT && Id<ID_TEXT+110)
					{	
						...
						//{
						//	PYSel = IdSel - ID_TEXT;
						//	WordSel[0] = t9pyfun.t9PY_addr;
						//	MULTIEDIT_AddText(hMulti,WordSel);
						//}
					}			
				break;
				...
				for(i=0;(i<ListBoxNum  || i< t9pyfun.mblen);i++)
				{
					if(i<t9pyfun.mblen)						//还没显示完拼音索引
					{
						if(ListBoxNum>t9pyfun.mblen)		//如果原listbox长度大于拼音索引数
							LISTBOX_SetString(hListBox,(t9pyfun.t9PY_addr+i)->PY,i);		//修改原有listbox项目的索引
						else								//如果原listbox长度不够
							LISTBOX_InsertString(hListBox,(t9pyfun.t9PY_addr+i)->PY,i);		//插入新的项目	
					}
					else	//删除多余的项目
...
//  GUI_FONT     Font;
  ...
  case WM_CREATE:
    ...
    for (i = 0; i < GUI_COUNTOF(_aButtonData); i++) {
	 ...
	 // BUTTON_SetBkColor(hButton,BUTTON_CI_PRESSED,_aButtonData[i].bkColor);
  ...
  case WM_TIMER:
	Id = WM_GetTimerId(pMsg->Data.v);	
 	TimerFlag = 1;
	//if((t9pyfun.pysrf == T9DX || t9pyfun.pysrf == T9XX) &&  c !='1')
	{
  ...
  case WM_NOTIFY_PARENT:
    ...
    case WM_NOTIFICATION_RELEASED:	
		...
		{
		//GUI_StoreKeyMsg(GUI_KEY_DELETE,1);
	
  //        GUI_StoreKeyMsg(GUI_KEY_DELETE, 0);
		...
		if(Id >=ID_BUTTON  && Id<(ID_BUTTON+GUI_COUNTOF(_aButtonData) ) )	//按键
		{
		 if (_aButtonData[Id - ID_BUTTON].acLabel) {	//确认是哪个按键
			 ...
			 {
				 {
					 ...
					 if(SamePress >= strlen(t9pyfun.t9PY_addr->PY))
						SamePress = 1;
					 else
						SamePress++;
			...
			/* 这部分没用了，在定时器消息处理里显示英文 */
			else if((t9pyfun.pysrf == T9DX || t9pyfun.pysrf == T9XX) &&  c !='1')
			{
				
				if(TimerFlag == 1);
				{
				//PYSel = Id - ID_BUTTON;
				WordSel[0] = t9pyfun.t9PY_addr->PY_mb[SamePress];
